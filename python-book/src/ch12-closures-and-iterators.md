...
> **你将学到什么：** 多行闭包（不仅仅是一行表达式的 lambda），`Fn`/`FnMut`/`FnOnce` 捕获语义，
...
print([f() for f in funcs])  # [2, 2, 2] —— 惊讶！都捕获了同一个 `i`
...
// `move` 为每个闭包捕获 `i` 的副本 —— 无延迟绑定惊讶。
```

### 三个闭包 Trait
```rust
// Rust 闭包实现这些 trait 中的一个或多个：
...
> **关键见解**：Rust 迭代器是惰性的 —— 直到 `.collect()` 之前什么都不发生。Python 的生成器工作方式相似，但列表推导式是急切求值的。
...
| `filter(f, iter)` | `.filter(f)` | 保留匹配元素 |
| `sum(iter)` | `.sum()` | 求和所有元素 |
| `min(iter)` / `max(iter)` | `.min()` / `.max()` | 返回 `Option` |
| `any(f(x) for x in iter)` | `.any(f)` | 如果任意匹配则为 true |
| `all(f(x) for x in iter)` | `.all(f)` | 如果全部匹配则为 true |
...
| `sorted(iter)` | 收集，然后 `.sort()` | 无惰性排序迭代器 |
...
```rust
// Rust —— 推导宏和声明宏用于代码生成
#[derive(Debug, Clone, PartialEq)]  // 在*编译时*生成 Debug、Clone、PartialEq 实现
struct Point {
    x: f64,
    y: f64,
}
...
// Default       → MyType::default()（类似无参数 __init__）

// Crate 提供的推导宏：
// Serialize, Deserialize (serde) → JSON/YAML/TOML 序列化
//                                  （类似 Python 的 json.dumps/loads 但是类型安全）
```

### Python 装饰器 vs Rust 推导

| Python 装饰器 | Rust 推导 | 用途 |
|--------------|----------|------|
...
<details>
<summary><strong>🏋️ 练习：推导和自定义 Debug</strong>（点击展开）</summary>

**挑战**：创建一个 `User` 结构体，字段为 `name: String`、`email: String` 和 `password_hash: String`。推导 `Clone` 和 `PartialEq`，但手动实现 `Debug` 以便打印姓名和电子邮件但隐藏密码（显示 `"***"` 而不是密码）。
...
**关键要点**：不像 Python 的 `__repr__`，Rust 允许你免费推导 `Debug` —— 但你可以为敏感字段覆盖它。这比 Python 更安全，Python 中 `print(user)` 可能意外泄露秘密。
