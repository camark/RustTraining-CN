...
> **你将学到什么：** 通过构建一个完整的 Rust CLI 应用程序，将课程中的所有内容联系在一起，
> Python 开发者通常会用 `argparse` + `json` + `pathlib` 编写这样的应用。
>
> **难度：** 🔴 高级

这个顶点项目练习了每个主要章节的概念：
- **第 3 章**：类型和变量（结构体、enum）
- **第 5 章**：集合（`Vec`、`HashMap`）
- **第 6 章**：Enum 和模式匹配（任务状态、命令）
- **第 7 章**：所有权和借用（传递引用）
- **第 9 章**：错误处理（`Result`、`?`、自定义错误）
- **第 10 章**：Traits（`Display`、`FromStr`）
...
> **Python 对比**：在 Python 中你会用 `@dataclass` + `Enum`。在 Rust 中，`struct` + `enum` + `derive` 宏免费给你序列化、显示和解析。
...
> **Python 对比**：Python 使用 `Path.read_text()` + `json.loads()`。Rust 使用 `fs::read_to_string()` + `serde_json::from_str()`。注意 `?` —— 每个错误都是显式的并传播。
...
> **Python 对比**：Python 使用 `argparse` 或 `click`。这个手工编写的解析器展示了如何在 enum 模式上使用 `match` 来替代 Python 的 if/elif 链。对于真实项目，使用 `clap` crate。
...
> **关键 Rust 模式**：`iter().map().max()`、`iter().filter().collect()`、`iter_mut().find()`、`retain()`、`iter().filter().count()`。这些替代了 Python 的列表推导式、`next(x for x in ...)` 和 `Counter`。
...
fn print_help() {
    println!("rustdo — 一个为学习 Rust 的 Pythonista 准备的任务管理器\n");
    println!("用法：");
    println!("  rustdo add <title> [low|medium|high]   添加任务");
    println!("  rustdo list [--all]                    列出待处理任务");
...
> **Python 等价物**：这是你的 `pyproject.toml` `[project.dependencies]`。`cargo add serde serde_json chrono dirs` 就像 `pip install`。
...
> **Python 等价物**：`pytest` 测试。使用 `cargo test` 而不是 `pytest`。不需要测试发现魔法 —— `#[test]` 显式标记测试函数。
...
1. **添加 `clap` 用于参数解析** —— 用 `clap` 的 derive 宏替手工编写的解析器：
   ```rust
   #[derive(Parser)]
   enum Command {
...
2. **添加彩色输出** —— 使用 `colored` crate 获得终端颜色（类似 Python 的 `colorama`）。

3. **添加截止日期** —— 添加 `Option<NaiveDate>` 字段并过滤逾期任务。

4. **添加标签/类别** —— 使用 `Vec<String>` 存储标签并用 `.iter().any()` 过滤。

5. **使其成为库 + 二进制** —— 拆分为 `lib.rs` + `main.rs` 以便逻辑可重用（第 8 章模块模式）。
...
| 第 7 章 | 所有权 + 借用 | `&[Task]` vs `Vec<Task>`、完成使用 `&mut` |
| 第 8 章 | 模块 | `mod task; mod storage; mod command; mod actions;` |
| 第 9 章 | 错误处理 | `Result<T, E>`、`?` 运算符、`.ok_or()` |
| 第 10 章 | Traits | `Display`、`FromStr`、`Serialize`、`Deserialize` |
| 第 11 章 | From/Into | `FromStr` 用于 Priority，错误转换的 `.into()` |
| 第 12 章 | 迭代器 | `filter`、`map`、`find`、`count`、`collect` |
| 第 14 章 | 测试 | `#[test]`、`#[cfg(test)]`、断言宏 |

> 🎓 **恭喜！** 如果你构建了这个项目，你已经使用了本书涵盖的每个主要 Rust 概念。你不再是一个学习 Rust 的 Python 开发者 —— 你是一个恰好也懂 Python 的 Rust 开发者。
