...
> **你将学到什么：** 默认不可变的变量，显式的 `mut`，原始数值类型 vs Python 的任意精度 `int`，
> `String` vs `&str`（早期最难的概念），字符串格式化，以及 Rust 必需的类型注解。
>
> **难度：** 🟢 初学者
...
# "常量" 只是约定：
MAX_SIZE = 1024    # 没有什么能阻止后面修改 MAX_SIZE = 999
```
...
// count = 5;            // ❌ 编译错误：无法对不可变变量进行两次赋值
// count = "hello";      // ❌ 编译错误：expected integer, found &str
...
// 在 Python 中，你只会重新赋值并丢失旧类型：
# input = "42"
# input = int(input)   # 同名，不同型 —— Python 也允许这个
# 但在 Rust 中，每个 `let` 创建真正的新绑定。旧的消失了。
```
...
> **关键差异**：在 Rust 中，方法签名中的 `&mut self` 告诉你（和编译器）`increment` 会修改计数器。在 Python 中，任何方法都可以修改任何东西 —— 你必须阅读代码才能知道。
...
let y: f64 = 3.14;         // 64 位 float（Python 的 float 等价物）
let big: i128 = 2_i128.pow(100); // 128 位最大值 —— 无任意精度
// 对于任意精度：使用 `num-bigint` crate

// 下划线用于可读性（类似 Python 的 1_000_000）：
let million = 1_000_000;   // 与 Python 相同的语法！
...
```rust
// Rust 推断类型，但它们是*固定*的 —— 不是动态的
let x = 42;          // 编译器推断为 i32（默认整数类型）
let y = 3.14;        // 编译器推断为 f64（默认 float 类型）
...
// 与 Python 不同，类型在推断后*不能*改变：
let x = 42;
// x = "hello";      // ❌ 错误：expected integer, found &str
```
...
这是 Python 开发者最大的惊讶之一。Rust 有**两种**主要字符串类型，而 Python 只有一种。

### Python 字符串处理
```python
# Python —— 一个字符串类型，不可变，引用计数
...
// 1. &str（字符串切片） —— 借用，不可变，类似字符串数据的"视图"
let name: &str = "Alice";           // 指向二进制文件中的字符串数据
                                     // 最接近 Python 的 str，但它是*引用*

// 2. String（_owned_ 字符串） —— 堆分配，可增长，拥有所有权
let mut greeting = String::from("Hello, ");  // 拥有，可以修改
...
// 关键要点：一些操作返回 &str（无分配），其他返回 String。
// .trim() 返回原始的切片 —— 高效！
...
pprint({"key": [1, 2, 3]})         # 美观打印
```

```rust
// Rust —— {:?} 和 {:#?}
println!("{:?}", vec![1, 2, 3]);       // "[1, 2, 3]" —— Debug 格式
println!("{:#?}", vec![1, 2, 3]);      // 美观打印的 Debug 格式
...
add(1, 2);         // ✅
// add("a", "b");  // ❌ 编译错误：expected i32, found &str
...
> 在 Rust 中，类型*是*程序 —— 编译器使用它们来保证内存安全，
> 防止数据竞争，并消除空指针错误。
>
> 📌 **另见**：[第 6 章 —— 枚举和模式匹配](ch06-enums-and-pattern-matching.md) 展示了 Rust 的类型系统如何替换 Python 的 `Union` 类型和 `isinstance()` 检查。
...
**挑战**：编写一个函数 `celsius_to_fahrenheit(c: f64) -> f64` 和一个函数 `classify(temp_f: f64) -> &'static str` 根据阈值返回 "cold"、"mild" 或 "hot"。打印 0、20 和 35 摄氏度的结果。使用字符串格式化。
...
**关键要点**：Rust 需要显式的 `f64`（无隐式 int→float），`for` 直接迭代数组（无 `range()`），`if/else` 块是表达式。
