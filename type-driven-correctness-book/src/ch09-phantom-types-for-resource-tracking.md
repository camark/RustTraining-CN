...
> **你将学到什么：** `PhantomData` 标记如何在类型级别编码寄存器宽度、DMA 方向和文件描述符状态 —— 以零运行时成本防止整个类别的资源不匹配 bug。
>
> **交叉引用**：[ch05](ch05-protocol-state-machines-type-state-for-r.md)（type-state）、[ch06](ch06-dimensional-analysis-making-the-compiler.md)（量纲类型）、[ch08](ch08-capability-mixins-compile-time-hardware-.md)（mixins）、[ch10](ch10-putting-it-all-together-a-complete-diagn.md)（集成）
...
**Phantom 类型**是出现在结构体定义中但不在任何字段中的类型参数。它纯粹存在于携带类型级信息：
...
DMA 缓冲区有方向：有些是**设备到主机**（读取），另一些是**主机到设备**（写入）。使用错误的方向会损坏数据或导致总线错误：
...
impl DmaBuffer<FromDevice> {
    ...
    pub fn read_data(&self) -> &[u8] {
        // SAFETY: ptr 对 self.len 字节有效，
        // 设备已完成写入（调用者确保 DMA 传输完成）。
...
//     buf.write_data(&[1, 2, 3]);  // ❌ DmaBuffer<FromDevice> 上没有方法 `write_data`
// }

// 不能从 ToDevice 缓冲区读取：
// fn oops2(buf: &DmaBuffer<ToDevice>) {
//     let data = buf.read_data();  // ❌ DmaBuffer<ToDevice> 上没有方法 `read_data`
// }
```
...
/// 组合 phantom 类型（寄存器宽度）与量纲类型（Celsius）。
fn read_temp_sensor(reg: &Register<Width16>) -> Celsius {
    let raw = reg.read();  // 保证 u16 由 phantom 类型
    Celsius(raw as f64 * 0.0625)  // 保证 Celsius 由返回类型
}

// 编译器强制执行：
// 1. 寄存器是 16 位（phantom 类型）
// 2. 结果是 Celsius（新类型）
// 两者都零运行时成本。
```
...
1. **PhantomData 以零大小携带类型级信息** —— 标记只存在于编译器。
2. **寄存器宽度不匹配成为编译错误** —— `Register<Width16>` 返回 `u16`，不是 `u32`。
3. **DMA 方向在结构上强制执行** —— `DmaBuffer<Read>` 没有 `write()` 方法。
4. **与量纲类型组合（ch06）** —— `Register<Width16>` 可以通过解析步骤返回 `Celsius`。
5. **Phantom 类型是编译时的** —— 它们不适用于运行时可变属性；对那些使用 enums。
