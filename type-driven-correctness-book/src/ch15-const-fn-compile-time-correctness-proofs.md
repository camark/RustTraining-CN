...
> **你将学到什么：** `const fn` 和 `assert!` 如何将编译器变成证明引擎 —— 在编译时验证 SRAM 内存映射、寄存器布局、协议帧、位域掩码、时钟树和查找表，零运行时成本。
>
> **交叉引用**：[ch04](ch04-capability-tokens-zero-cost-proof-of-aut.md)（能力令牌）、[ch06](ch06-dimensional-analysis-making-the-compiler.md)（量纲分析）、[ch09](ch09-phantom-types-for-resource-tracking.md)（phantom 类型）

## 问题：虚假的内存映射

在嵌入式和系统编程中，内存映射是一切的基础 —— 它们定义了引导加载程序、固件、数据段和栈的位置。边界错误，两个子系统会静默地相互损坏。在 C 中，这些映射通常是 `#define` 常量，没有结构关系：
...
#define DATA_SIZE       (80 * 1024)     /* 有人把这段从 64K 调到 80K */

#define STACK_BASE      0x20038000
#define STACK_SIZE      (48 * 1024)     /* 0x20038000 + 48K = 0x20044000 —— 超出 SRAM 结尾！ */
```

Bug：`16 + 128 + 80 + 48 = 272 KB`，但 SRAM 只有 256 KB。栈超出物理内存结尾 16 KB。没有编译器警告，没有链接器错误，没有运行时检查 —— 只有当栈增长到未映射空间时的静默损坏。

**每个失败模式都是在部署后发现的** —— 可能表现为仅在重栈使用时发生的莫名崩溃，在数据段调整大小几周后。
...
> **关键见解：** `const fn` + `assert!` = 证明义务。每个断言是编译器必须验证的定理。如果证明失败，程序无法编译。不需要测试套件，不需要代码审查捕获 —— 编译器本身是审计员。

## 构建验证的 SRAM 内存映射

### Region 类型

`Region` 表示连续的内存块。它的构造函数是 `const fn`，强制执行基本有效性：
...
现在我们将区域组合成完整的 SRAM 映射。构造函数证明六个无重叠不变量和四个包含不变量 —— 全都在编译时：
...
十个编译时检查，零运行时指令。二进制只包含验证的常量。

### 破坏映射

假设有人将数据段从 64 KB 增加到 80 KB 而不调整其他东西：
...
const BAD_SRAM: SramMap = SramMap::verified(
    ...
    Region::new(0x2003_8000,  48 * 1024),   // 栈被推到 SRAM 结尾之外
);
```
...
> **原本是神秘现场故障的 bug 现在是编译错误。** 不需要单元测试，不需要代码审查捕获 —— 编译器证明它不可能。与 C 比较，相同的 bug 会静默发布，并在数月后作为栈损坏在现场出现。
...
fn read_word<A>(region: &TypedRegion<A>, offset: u32) -> u32 {
    ...
    // 在真正的固件中：unsafe { core::ptr::read_volatile((region.base + offset) as *const u32) }
    0 // stub
}

// 写需要 ReadWrite —— 函数签名强制执行它
fn write_word(region: &TypedRegion<ReadWrite>, offset: u32, value: u32) {
    assert!(offset + 4 <= region.size, "write out of bounds");
    // 在真正的固件中：unsafe { core::ptr::write_volatile(...) }
    let _ = value; // stub
}
...
fn main() {
    read_word(&BOOTLOADER, 0);      // ✅ 从只读区域读
    read_word(&DATA, 0);            // ✅ 从读写区域读
    write_word(&DATA, 0, 42);       // ✅ 写入读写区域
    // write_word(&BOOTLOADER, 0, 42); // ❌ 编译错误：期望 ReadWrite，找到 ReadOnly
}
```

引导加载程序区域在物理上可写（它是 SRAM），但类型系统防止意外写入。**硬件能力**和**软件权限**之间的这种区别正是正确构造的含义。

## 指针溯源：证明地址属于区域

更进一步，我们可以创建验证的地址 —— 在静态上被证明位于特定区域内的值：
...
**在编译时建立溯源** —— 访问这些地址时不需要运行时边界检查。构造函数是私有的，所以 `VerifiedAddr` 只能存在如果编译器已证明它有效。

## 超越内存映射

`const fn` 证明模式适用于任何你有**编译时已知值和结构不变量**的地方。上面的 SRAM 映射证明了*区域间*属性（包含、无重叠）。相同的技术可以扩展到越来越细粒度的域：
...
每个小节遵循相同的模式：定义一个类型，带有编码不变量的 `const fn` 构造函数，然后使用 `const _: () = { ... }` 或 `const` 绑定来触发验证。

### 寄存器映射

硬件寄存器块有固定的偏移和宽度。错位或重叠的寄存器定义总是 bug：
...
重用上面 UART 示例的 `Register` 和 `disjoint` 函数。定义三个或四个 `const Register` 值（例如，`CTRL` 在偏移 0x00 宽度 4，`STATUS` 在 0x04 宽度 4，`TX_DATA` 在 0x08 宽度 1，`RX_DATA` 在 0x0C 宽度 1）并断言这三个属性。
...
字段通过构造是连续的 —— 每个字段正好在前一个字段结束处开始。最终断言证明帧适合协议的最大大小。

### 用于泛型验证的内联 Const 块

从 Rust 1.79 开始，`const { ... }` 块让你在使用点验证 const 泛型参数 —— 完美适用于 DMA 缓冲区大小约束或对齐要求：
...
断言在函数单态化时求值 —— 每个带有不同 `N` 的调用点获得自己的编译时检查。

### 寄存器内的位域布局

寄存器映射证明寄存器不*相互重叠* —— 但**单个寄存器内的位**呢？控制寄存器将多个字段打包到一个字中。如果两个字段共享位位置，读和写会静默相互损坏。在 C 中，这通常通过手动审查掩码常量捕获（或没有）。
...
这补充了上面的寄存器映射模式 —— 寄存器映射证明*寄存器间*的不相交性，而位域布局证明*寄存器内*的不相交性。它们一起提供从寄存器块到单个位的完整覆盖。

### 时钟树 / PLL 配置

微控制器通过乘法器/除法器链派生外设时钟。PLL 产生 `f_vco = f_in × N / M`，且 VCO 频率必须保持在硬件指定的范围内。为特定板获取任何参数错误，芯片输出垃圾时钟或拒绝锁定。这些约束是 `const fn` 的完美目标：
...
编译器在*中间*推导链中捕获约束违规 —— 不是在结尾。如果你改为违反系统时钟限制（`sysclk > 168 MHz`），错误消息将指向该断言。

> **派生值约束链将单个 `const fn` 变成多阶段证明。** 每个中间值有自己的硬件强制范围。更改一个参数（例如，换成 25 MHz 晶体）立即暴露任何下游违规。

**派生值约束链** —— VCO 频率取决于 `input / m × n`，系统时钟取决于 `vco / p`。每个中间值有自己的硬件强制范围。单个 `const fn` 验证整个链，所以更改一个参数（例如，换成 25 MHz 晶体）立即暴露任何下游违规。
...
`crc32_table()` 函数完全在编译期间运行。生成的 1 KB 表被嵌入到二进制的只读数据段 —— 无分配器，无初始化代码，无启动成本。与 C 方法比较，C 方法要么使用代码生成器，要么在启动时计算表。Rust 版本被证明是正确的（`const _` 断言验证已知值）且被证明是完整的（如果函数无法生成有效的表，编译器将拒绝程序）。
...
| 寄存器内的位域掩码 | ✅ 总是 |
| 时钟树 / PLL 参数链 | ✅ 总是 |
| 查找表（CRC、三角函数、编码） | ✅ 总是 —— 零启动成本 |
| 带有交叉值不变量的常量（无重叠、总和 ≤ 边界） | ✅ 总是 |
| 带有域约束的配置值 | ✅ 当值在编译时已知 |
| 从用户输入或文件计算的值 | ❌ 使用运行时验证 |
| 高度动态结构（树、图） | ❌ 使用基于属性的测试 |
| 单值范围检查 | ⚠️  考虑新类型 + `From` 替代（[ch07](ch07-validated-boundaries-parse-dont-validate.md)） |

### 成本总结

| 什么 | 运行时成本 |
|------|:------:|
| `const fn` 断言（`assert!`、`panic!`） | 仅编译时 —— 0 指令 |
| `const _: () = { ... }` 验证块 | 仅编译时 —— 不在二进制中 |
| `Region`、`Register`、`Field` 结构体 | 纯数据 —— 与原始整数相同的布局 |
| 内联 `const { }` 泛型验证 | 在编译时单态化 —— 0 成本 |
| 查找表（`crc32_table()`） | 在编译时计算 —— 放在 `.rodata` |
| Phantom 类型访问标记（`TypedRegion<RW>`） | 零大小 —— 优化掉 |

每行都是**零运行时成本** —— 证明只存在于编译期间。生成的二进制只包含验证的常量和查找表，没有断言检查代码。

## 练习：Flash 分区映射

为从 `0x0800_0000` 开始的 1 MB NOR flash 设计验证的 flash 分区映射。要求：

1. 四个分区：**bootloader**（64 KB）、**application**（640 KB）、**config**（64 KB）、**OTA staging**（256 KB）
2. 每个分区必须**4 KB 对齐**（flash 擦除粒度）：基址和大小必须是 4096 的倍数
3. 没有分区可以重叠另一个
4. 所有分区必须适合 flash
5. 添加 `const fn total_used()` 返回所有分区大小的总和并断言它等于 1 MB
...
2. **内存映射是理想的候选** —— 子区域包含、无重叠、总大小边界和对齐约束都可以表示为 const fn 断言。C `#define` 方法不提供这些保证。

3. **Phantom 类型分层在上** —— 结合 const fn（值验证）与 phantom 类型访问标记（权限验证），零运行时成本深度防御。

4. **溯源可以在编译时建立** —— `VerifiedAddr` 在编译时证明地址属于特定区域，消除每次访问时的运行时边界检查。

5. **模式泛化到内存之外** —— 寄存器映射、位域掩码、协议帧、时钟树、DMA 参数 —— 任何你有编译时已知值和结构不变量的地方。

6. **位域和时钟树是理想的候选** —— 寄存器内位不相交性和派生值约束链（VCO 范围、除法器限制）正是 `const fn` 轻松证明的那种不变量。

7. **`const fn` 取代代码生成器和构建脚本用于查找表** —— CRC 表、三角函数、编码映射 —— 在编译时计算，放在 `.rodata` 中，零启动成本，无外部工具。

8. **内联 `const { }` 块验证泛型参数** —— 从 Rust 1.79 开始，你可以在调用点强制执行 const 泛型约束，在任何代码运行前捕获误用。
