...
> **你将学到什么：** 如何将 type-state 会话、能力令牌、phantom 类型资源导航、量纲分析、验证边界、builder type-state 和一次性类型组合成完整的、零开销的 Redfish 客户端 —— 每个协议违规都是编译错误。
>
> **交叉引用**：[ch02](ch02-typed-command-interfaces-request-determi.md)（类型化命令）、[ch03](ch03-single-use-types-cryptographic-guarantee.md)（一次性类型）、[ch04](ch04-capability-tokens-zero-cost-proof-of-aut.md)（能力令牌）、[ch05](ch05-protocol-state-machines-type-state-for-r.md)（type-state）、[ch06](ch06-dimensional-analysis-making-the-compiler.md)（量纲类型）、[ch07](ch07-validated-boundaries-parse-dont-validate.md)（验证边界）、[ch09](ch09-phantom-types-for-resource-tracking.md)（phantom 类型）、[ch10](ch10-putting-it-all-together-a-complete-diagn.md)（IPMI 集成）、[ch11](ch11-fourteen-tricks-from-the-trenches.md)（技巧 4 —— builder type-state）

## 为什么 Redfish 值得拥有独立章节

第 10 章围绕 IPMI 组合核心模式 —— 一个字节级协议。但大多数 BMC 平台现在除了（或代替）IPMI 外还暴露**Redfish** REST API，Redfish 引入了它自己类别的正确性危险：

| 危险 | 示例 | 后果 |
|--------|---------|-------------|
| 格式错误的 URI | `GET /redfish/v1/Chassis/1/Processors`（错误的父级） | 404 或错误数据静默返回 |
| 错误电源状态上的操作 | 在已关闭系统上调用 `Reset(ForceOff)` | BMC 返回错误，或者更糟，与另一个操作竞争 |
| 缺少特权 | 操作员级代码调用 `Manager.ResetToDefaults` | 生产环境中 403，安全审计发现 |
| 不完整的 PATCH | 从 PATCH body 中省略必需的 BIOS 属性 | 静默无操作或部分配置损坏 |
| 未验证的固件应用 | 在镜像完整性检查之前调用 `SimpleUpdate` | BMC 变砖 |
| 模式版本不匹配 | 在 v1.5 BMC 上访问 `LastResetTime`（v1.13 添加） | `null` 字段 → 运行时 panic |
| 遥测单位混淆 | 比较入口温度（°C）与功耗（W） | 无意义的阈值决策 |

在 C、Python 或未类型化的 Rust 中，这些每一个都仅通过纪律和测试防止。本章使它们成为**编译错误**。

## 未类型化的 Redfish 客户端

典型的 Redfish 客户端看起来像这样：
...
这"有效" —— 直到它失效。每个 `unwrap()` 是潜在的 panic，每个字符串路径是未检查的假设，单位混淆是看不见的。
...
//   ❌ 错误：未找到 `RedfishSession<Connected>` 的方法 `http_get`
...
**消除的 bug 类别：** 在未连接或未认证的会话上发送请求。方法根本不存在 —— 没有运行时需要忘记的检查。

---

## 第 2 节 —— 特权令牌（能力令牌，ch04）

Redfish 定义四个特权级别：`Login`、`ConfigureComponents`、`ConfigureManager`、`ConfigureSelf`。不是在运行时检查权限，而是将它们编码为零大小证明令牌。
...
**消除的 bug 类别：** 特权升级。操作员级登录物理上无法产生 `ConfigureManagerToken` —— 编译器不会让代码引用它。零运行时成本：对于编译的二进制，这些令牌不存在。

---

## 第 3 节 —— 类型化资源导航（Phantom 类型，ch09）

Redfish 资源形成树。将层次结构编码为类型防止构造非法 URI：
...
**消除的 bug 类别：** 格式错误的 URI、导航到给定父级下不存在的子资源。层次结构在结构上强制执行 —— 你只能通过 `Chassis → Instance → Thermal` 到达 `Thermal`。
...
// ──── 类型化 Redfish GET（应用到 REST 的 ch02 模式）────
...
## 第 5 节 —— 带 Builder Type-state 的 PATCH（ch11，技巧 4）

Redfish PATCH payload 必须包含特定字段。一个 builder 在设置必需字段之前门控 `.apply()`，防止不完整或空的 patch：
...
impl<T> BiosPatchBuilder<FieldUnset, T> {
    /// 设置启动顺序 —— 转换 BootOrder 标记到 FieldSet。
...
impl<B> BiosPatchBuilder<B, FieldUnset> {
    /// 设置 TPM 状态 —— 转换 TpmState 标记到 FieldSet。
...
## 第 6 节 —— 固件更新生命周期（一次性 + Type-state，ch03 + ch05）

Redfish `UpdateService` 有严格的序列：push image → verify → apply → reboot。每个阶段必须恰好发生一次，按顺序。
...
impl FirmwareUpdate<FwIdle> {
    pub fn push_image(
        ...
        // 或多部分 push 到 /redfish/v1/UpdateService/upload
        let _ = image;
...
**消除的 bug 类别：**
- **应用未验证的固件：** `.apply()` 仅存在于 `FwVerified`。
- **双重应用：** `apply()` 消耗 `self` —— move 的值不能重用。
- **跳过重启：** `FwNeedsReboot` 是不同的类型；你不能意外地在固件暂存时继续正常操作。
...
| # | Bug 类别 | 如何防止 | 模式（章节） |
|---|-----------|-------------------|-------------------|
| 1 | 未认证会话上的请求 | `http_get()` 仅存在于 `Session<Authenticated>` | Type-state（§1） |
| 2 | 特权升级 | `ConfigureManagerToken` 不被操作员登录返回 | 能力令牌（§2） |
| 3 | 格式错误的 Redfish URI | 导航方法强制执行父→子层次结构 | Phantom 类型（§3） |
...
| 7 | 不完整的 PATCH payload | `.apply()` 仅在所有字段是 `FieldSet` 时存在 | Builder type-state（§5） |
| 8 | PATCH 缺少特权 | `.apply()` 需要 `&ConfigureComponentsToken` | 能力令牌（§5） |
| 9 | 应用未验证的固件 | `.apply()` 仅存在于 `FwVerified` | Type-state（§6） |
| 10 | 双重固件应用 | `apply()` 消耗 `self` —— 值被 move | 一次性类型（§6） |
| 11 | 无权威的固件更新 | `push_image()` 需要 `&ConfigureManagerToken` | 能力令牌（§6） |
| 12 | 注销后使用 | `logout()` 消耗会话 | 所有权（§1） |

**所有十二个保证的总运行时开销：零。**

生成的二进制与未类型化版本进行相同的 HTTP 调用 —— 但未类型化版本可能有 12 类 bug。这个版本不能。

---

## 比较：IPMI 集成（ch10）vs. Redfish 集成
...
| Payload 构造 | 字节数组 | JSON 的 Builder type-state |
| 更新生命周期 | 未覆盖 | 完整 type-state 链 |
| 练习的模式 | 7 | 8（添加 builder type-state） |

两章是互补的：ch10 展示模式在字节级有效，本章展示它们在 REST/JSON 级同样有效。类型系统不关心传输 —— 它证明任一方式的正确性。

## 关键要点

1. **八个模式组合成一个 Redfish 客户端** —— 会话 type-state、能力令牌、phantom 类型 URI、类型化命令、量纲分析、验证边界、builder type-state 和一次性固件应用。
2. **十二类 bug 成为编译错误** —— 见上表。
3. **零运行时开销** —— 每个证明令牌、phantom 类型和 type-state 标记编译消失。二进制与手写未类型化代码相同。
4. **REST API 与字节协议一样受益** —— 来自 ch02–ch09 的模式同样适用于 JSON-over-HTTPS（Redfish）和 bytes-over-KCS（IPMI）。
5. **特权强制执行是结构的，不是过程的** —— 函数签名声明需要什么；编译器强制执行它。
6. **这是设计模板** —— 为你的特定 Redfish 模式和组织角色层次结构调整资源类型标记、能力令牌和 builder。
