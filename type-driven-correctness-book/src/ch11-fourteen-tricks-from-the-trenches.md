...
> **你将学到什么：** 十四个较小的正确构建技术 —— 从哨兵消除和密封 trait 到会话类型、`Pin`、RAII 和 `#[must_use]` —— 每个都消除特定的 bug 类别，几乎零努力。
>
...
八个核心模式（ch02–ch09）涵盖了主要的正确构建技术。
本章收集了**十四个较小但高价值的技巧**，它们在
生产 Rust 代码中反复出现 —— 每个都消除特定类别的 bug，几乎零努力。

### 技巧 1 —— 在边界将哨兵转换为 `Option`

硬件协议充满了哨兵值：IPMI 使用 `0xFF` 表示"传感器不存在"，PCI 使用 `0xFFFF` 表示"无设备"，SMBIOS 使用 `0x00` 表示"未知"。如果你将这些哨兵作为普通整数在代码中传递，每个消费者都必须记住检查这个魔术值。如果有一个比较忘记了，你会得到幻影 255 °C 读数或虚假的供应商 ID 匹配。

**规则：** 在第一个解析边界将哨兵转换为 `Option`，并且*只在*序列化边界转换回哨兵。
...
current_vendor_id = u16::from_str_radix(hex, 16)
    .unwrap_or(0xFFFF);  // 哨兵隐藏错误
```

每个接收 `current_vendor_id` 的函数都必须知道 `0xFFFF` 是特殊的。如果有人编写 `if vendor_id == target_id` 而没有先检查 `0xFFFF`，当目标也碰巧从坏输入解析为 `0xFFFF` 时，丢失的设备会静默匹配。
...
现在每个消费者*必须*处理 `None` 情况 —— 编译器强制执行它：
...
//     temp > threshold  // 错误：不能比较 Option<u8> 与 u8
// }
```

#### 真实影响

`inventory/src/events.rs` 为 GPU 热警报使用相同的模式：
```rust,ignore
...
第 2 章介绍了 `IpmiCmd`，带有一个关联类型，将每个命令绑定到其响应。但有一个漏洞：如果*任何*代码可以实现 `IpmiCmd`，有人可以编写一个 `MaliciousCmd`，其 `parse_response` 返回错误的类型或 panic。整个系统的类型安全依赖于每个实现的正确性。

**密封 trait** 堵塞了这个漏洞。想法很简单：使 trait 需要一个*私有*的超 trait，只有你的 crate 可以实现。
...
| 密封当… | 不密封当… |
|-----------|-----------------|
| 安全依赖于正确的实现（IpmiCmd、DiagModule） | 用户应该扩展系统（自定义报告格式化程序） |
| 关联类型必须满足不变量 | trait 是简单的能力标记（HasIpmi） |
| 你拥有规范的实现集 | 第三方插件是设计目标 |

#### 真实候选者

- `IpmiCmd` —— 不正确的解析可能污染类型化响应
- `DiagModule` —— 框架假设 `run()` 返回有效的 DER 记录
- `SelEventFilter` —— 损坏的过滤器可能吞没关键的 SEL 事件

***

### 技巧 3 —— 用于进化 Enums 的 `#[non_exhaustive]`
...
当下一代出货且你添加 `S4001` 时，任何外部代码匹配 `SkuVariant` 且没有通配符分支的将**静默失败无法编译** —— 这就是重点。但内部代码呢？没有 `#[non_exhaustive]`，你在*同一个 crate* 中的 `match` 编译时不需要通配符，添加新变体会破坏你自己的构建。

标记 enum 为 `#[non_exhaustive]` 强制**外部 crate** 在其上匹配时包含通配符分支。在定义 crate 内，`#[non_exhaustive]` 没有效果 —— 你仍然可以编写穷尽匹配。

**为什么这有用：** 当你从库 crate（或工作区中的共享子 crate）发布 `SkuVariant` 时，下游代码被迫处理未知的未来变体。当你下一代添加 `S4001` 时，下游代码已经编译 —— 它们有通配符分支。

```rust,ignore
// 在 gpu_sel crate 中（定义 crate）：
#[non_exhaustive]
...
fn diag_path_external(sku: inventory::SkuVariant) -> &'static str {
    match sku {
        ...
        _ => "generic_diag",  // #[non_exhaustive] 为外部 crate 要求
    }
}
```

> **工作区提示：** 如果所有代码都在单个 crate 中，`#[non_exhaustive]`
> 没有帮助 —— 它只影响跨 crate 边界。对于项目的
> 大型工作区，将进化的 enums 放在共享 crate（`core_lib` 或
> `inventory`）中，以便属性保护其他工作区 crate 中的消费者。
...
第 5 章展示了 type-state 用于*协议*（会话生命周期、链路训练）。
同样的想法适用于*builders* —— 结构体的 `build()` / `finish()`
只能在设置所有必填字段时调用。
...
#### 何时使用 typestate builders

| 使用当… | 不费心当… |
|-----------|-------------------|
| 省略字段导致静默 bug（DER 缺少 mnemonic） | 所有字段都有合理的默认值 |
| builder 是公共 API 的一部分 | builder 仅是测试支架 |
| 超过 2-3 个必填字段 | 单个必填字段（只是在 `new()` 中接受它） |

***

### 技巧 5 —— `FromStr` 作为验证边界

第 7 章展示了 `TryFrom<&[u8]>` 用于二进制数据（FRU 记录、SEL 条目）。
对于**字符串**输入 —— 配置文件、CLI 参数、JSON 字段 —— 类似的边界是 `FromStr`。
...
带有 `"diag_level": "extendedd"`（拼写错误）的配置文件静默获得 `QuickMode`。
...
1. **快速失败：** 坏输入在解析边界被捕获，不是在诊断逻辑深处三层。
2. **别名是显式的：** `"MEM"`、`"DIMM"` 和 `"MEMORY"` 都映射到 `Component::Memory` —— 匹配分支记录了映射。
3. **`.parse()` 是符合人体工程学的：** 因为 `FromStr` 与 `str::parse()` 集成，你获得简洁的单行代码：`let level: DiagLevel = config["level"].parse()?;`
...
| `SkuVariant` | `inventory` | 相同别名（单独的模块，相同的模式） |
| `FaultStatus` | `config_loader` | 故障生命周期状态 |
| `DiagAction` | `config_loader` | 修复动作类型 |
...
| 章节 | ch07 | ch11 |
| 两者使用 | `Result` —— 强制调用者处理无效输入 |

***

### 技巧 6 —— Const Generics 用于编译时大小验证

当硬件缓冲区、寄存器组或协议帧有固定大小时，
const generics 让编译器强制执行它们：
...
**带有 const generics 的编译时断言：**

```rust,ignore
/// NVMe 管理命令使用 4096 字节缓冲区。编译时强制执行。
...
// NvmeBuffer::<1024>::new();  // 此形式在运行时 panic
// 真正的编译时强制执行，见技巧 9（const 断言）。
```

> **何时使用：** 固定大小的协议缓冲区（NVMe、PCIe 配置空间）、
> DMA 描述符、硬件 FIFO 深度。任何大小是硬件常量的地方，不应该在运行时变化。

***

### 技巧 7 —— `unsafe` 周围的安全包装器

项目当前有零个 `unsafe` 块。但当你
添加 MMIO 寄存器访问、DMA 或 FFI 到 accel-mgmt/accel-query 时，你需要
`unsafe`。正确构建方法：**每个 `unsafe` 块包装在安全抽象中**，以便不安全性被包含和可审计。

```rust,ignore
/// MMIO 映射的寄存器。指针对映射的生命周期有效。
/// 所有 unsafe 包含在此模块中 —— 调用者使用安全方法。
...
impl MmioRegion {
    ...
    pub fn read_u32(&self, offset: usize) -> Option<u32> {
        ...
        // SAFETY: 偏移量在上面边界检查，base 根据 new() 契约有效
        Some(unsafe {
...
**与 phantom 类型组合（ch09）用于类型化 MMIO：**
...
impl TypedMmio<ReadOnly> {
    ...
    // 没有 write 方法 —— 如果你尝试写入 ReadOnly 区域会编译错误
}
...
> | 规则 | 为什么 |
> |------|-----|
> | 一个带有文档 `# Safety` 不变量的 `unsafe fn new()` | 调用者一次承担责任 |
> | 所有其他方法是安全的 | 调用者不能触发 UB |
> | 每个 `unsafe` 块上的 `# SAFETY:` 注释 | 审计者可以在本地验证 |
> | 用 `#[deny(unsafe_op_in_unsafe_fn)]` 包装在模块中 | 即使在 `unsafe fn` 内，单个操作也需要 `unsafe` |
...
你现在有七个日常技巧。这是一个快速的记分卡：

| 技巧 | 消除的 bug 类别 | 采用努力 |
|:-----:|----------------------|:---------------:|
| 1 | 哨兵混淆（0xFF） | 低 —— 边界上一个 `match` |
| 2 | 未授权的 trait 实现 | 低 —— 添加 `Sealed` 超 trait |
| 3 | enum 增长后的破坏消费者 | 低 —— 一行属性 |
| 4 | 缺少 builder 字段 | 中 —— 额外类型参数 |
| 5 | 字符串类型配置中的拼写错误 | 低 —— `impl FromStr` |
| 6 | 错误的缓冲区大小 | 低 —— const generic 参数 |
| 7 | unsafe 散布在代码库中 | 中 —— 包装器模块 |

技巧 8-14 **更高级** —— 它们涉及 async、const 求值、会话类型、`Pin` 和 `Drop`。如果需要的话在这里休息一下；上面的技巧已经是高价值、低努力的胜利，你明天就可以采用。

***

### 技巧 8 —— Async Type-State Machines

当硬件驱动程序使用 `async`（例如，async BMC 通信、async NVMe I/O）时，type-state 仍然有效 —— 但在 `.await` 点之间的所有权需要小心：
...
impl AsyncSession<Idle> {
    ...
    /// Session 被消耗（移动进 future）跨越 .await。
...
| 规则 | 为什么 |
|------|-----|
| 转换方法接受 `self`（按值），不是 `&mut self` | 所有权转移在 `.await` 之间有效 |
| 返回 `Result<NextState, (Error, PrevState)>` 用于可恢复错误 | 调用者可以从先前状态重试 |
| 不要将状态拆分到多个 futures | 一个 future 拥有一个 session |
| 如果使用 tokio::spawn 使用 `Send + 'static` 边界 | session 必须可以在线程间移动 |

> **警告：** 如果你需要在错误时取回*先前*状态（重试），
> 返回 `Result<AsyncSession<Active>, (Error, AsyncSession<Idle>)>` 以便
> 调用者拿回所有权。没有这个，失败的 `.await` 会永久丢弃 session。

***

### 技巧 9 —— 通过 Const 断言的 Refinement 类型

当数字约束是编译时不变量（不是运行时数据）时，
使用 `const` 求值来强制执行它。这与技巧 6（提供类型级大小区分）不同 —— 这里我们在编译时*拒绝无效值*：
...
// read_sensor_const::<0x20>();   // ✅ 编译 —— 0x20 有效
...
> **何时使用：** 编译时已知的硬件定义固定 ID（传感器 ID、风扇插槽、PCIe 插槽号）。当值来自运行时数据（配置文件、用户输入）时，改用 `TryFrom` / `FromStr`（ch07，技巧 5）。

***

### 技巧 10 —— 用于通道通信的会话类型

当两个组件通过通道通信时（例如，诊断编排器 ↔ 工作线程），**会话类型**在类型系统中编码协议：

```rust,ignore
use std::marker::PhantomData;

// 协议：Client 发送 Request，Server 发送 Response，然后完成。
...
/// 类型化的通道端点。`S` 是当前协议状态。
pub struct Chan<S> {
    // 在真实代码中：包装 mpsc::Sender/Receiver 对
    _state: PhantomData<S>,
}
...
> **何时使用：** 线程间诊断协议、BMC 命令序列、任何顺序重要的请求 - 响应模式。对于复杂的多消息协议，考虑 [`session-types`](https://crates.io/crates/session-types) 或 [`rumpsteak`](https://crates.io/crates/rumpsteak) crates。

***

### 技巧 11 —— 用于自引用状态机的 `Pin`

某些 type-state 机器需要持有对自己数据的引用（例如，跟踪其拥有缓冲区内位置的解析器）。Rust 通常禁止这个，因为移动结构体会使内部指针无效。`Pin<T>` 通过保证值**不会移动**来解决这个问题：
...
impl StreamParser {
    ...
    /// 读取下一个字节 —— 只能通过 Pin<&mut Self> 调用。
    pub fn next_byte(self: Pin<&mut Self>) -> Option<u8> {
        // 解析器不能移动，所以 cursor 保持有效
...
**关键洞察：** `Pin` 是自引用结构体问题的正确构建解决方案。没有它，你需要 `unsafe` 和手动生命周期跟踪。有了它，编译器防止移动，内部指针不变量得以维持。

| 使用 `Pin` 当… | 不使用 `Pin` 当… |
|-----------------|----------------------|
| 状态机持有结构体内引用 | 所有字段都是独立拥有的 |
| 在 `.await` 之间借用的 async futures | 不需要自引用 |
| 必须在内存中重定位的 DMA 描述符 | 数据可以自由移动 |
| 带有内部 cursor 的硬件环缓冲区 | 简单的基于索引的迭代有效 |

***

### 技巧 12 —— RAII / `Drop` 作为正确性保证

Rust 的 `Drop` trait 是一个正确构建机制：清理代码**不能被忘记**，因为编译器自动插入它。这对于必须正好释放一次的硬件资源特别有价值。

```rust,ignore
use std::io;

/// 完成后*必须*关闭的 IPMI 会话。
/// `Drop` impl 保证清理，即使在 panic 或早期 `?` 返回时。
...
**将 RAII 与 type-state 组合（ch05）用于有序清理：**

你不能在泛型参数上特化 `Drop`（Rust 错误 E0366）。
相反，使用**每个状态的单独包装类型**：
...
impl Drop for LockedGpu {
    fn drop(&mut self) {
        // 在 drop 时解锁时钟 —— 只为锁定包装器触发。
        eprintln!("[RAII] GPU {} clocks unlocked", self.device_id);
    }
}

// GpuContext<Open> 没有特殊的 Drop —— 没有时钟要解锁。
// LockedGpu 总是在 drop 时解锁，即使在 panic 或早期返回时。
```
...
> | 方法 | 优点 | 缺点 |
> |----------|------|------|
> | 单独的包装类型（上面） | 干净、零成本 | 额外的类型名 |
> | 泛型 `Drop` + 运行时 `TypeId` 检查 | 单个类型 | 需要 `'static`，运行时成本 |
> | `Drop` 中带有穷尽匹配的 `enum` 状态 | 单个泛型类型 | 运行时分发，类型安全性较低 |

> **何时使用：** BMC 会话、GPU 时钟锁定、DMA 缓冲区映射、文件句柄、互斥锁保护、任何带有强制释放步骤的资源。如果你发现自己编写 `fn close(&mut self)` 或 `fn cleanup()`，它几乎肯定应该是 `Drop`。

***

### 技巧 13 —— 作为正确性的错误类型层次结构

设计良好的错误类型防止静默错误吞没，确保调用者适当地处理每个失败模式。使用 `thiserror` 进行结构化错误处理是一个正确构建的模式：编译器强制穷尽匹配。
...
| 日志吞没错误 | `match` 强制处理每种情况 |
| 新错误变体 → 没人注意 | 新变体 → 编译器警告不匹配的分支 |
...
|-----------------------|-------------------|
| 编写库/crate | 编写二进制/CLI |
| 调用者需要匹配错误变体 | 调用者只是日志和退出 |
| 错误类型是公共 API 的一部分 | 内部错误管道 |
| `protocol_lib`、`accel_diag`、`thermal_diag` | `diag_tool` 主二进制 |

> **何时使用：** 工作区中的每个 crate 都应该用 `thiserror` 定义自己的错误 enum。顶层二进制 crate 可以使用 `anyhow` 来聚合它们。这给库调用者编译时错误处理保证，同时保持二者的易用性。

***

### 技巧 14 —— 用于强制执行消费的 `#[must_use]`

`#[must_use]` 属性将忽略的返回值转换为编译器警告。这是一个轻量级的正确构建工具，与本指南中的每个模式配对：

```rust,ignore
/// 必须使用的校准令牌 —— 静默丢弃它是 bug。
...
| 模式 | 注释什么 | 为什么 |
|---------|-----------------|-----|
| 一次性令牌（ch03） | `CalibrationToken`、`FusePayload` | 不使用而丢弃 = 逻辑 bug |
| 能力令牌（ch04） | `AdminToken` | 认证但忽略令牌 |
| Type-state 转换 | `authenticate()`、`activate()` 的返回类型 | 创建但从未使用的会话 |
| 结果 | `DiagResult`、`SensorReading` | 静默失败吞没 |
| RAII 句柄（技巧 12） | `IpmiSession`、`LockedGpu` | 打开但不使用资源 |

> **经验法则：** 如果不使用而丢弃值总是 bug，添加 `#[must_use]`。如果有时是故意的（例如，`Vec`），不要。`_` 前缀（`let _ = foo()`）显式确认并消除警告 —— 这在丢弃是故意时没问题。

## 关键要点

1. **在边界哨兵 → Option** —— 解析时将魔术值转换为 `Option`；编译器强制调用者处理 `None`。
2. **密封 trait 关闭实现漏洞** —— 私有超 trait 意味着只有你的 crate 可以实现 trait。
3. **`#[non_exhaustive]` + `#[must_use]` 是一行、高价值的注释** —— 将它们添加到进化的 enums 和消耗的令牌。
4. **Typestate builders 强制执行必填字段** —— `finish()` 仅在所有必填类型参数为 `Set` 时存在。
5. **每个技巧针对特定的 bug 类别** —— 逐步采用它们；没有技巧需要重写你的架构。
