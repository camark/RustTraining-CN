...
> **你将学到什么：** Rust 的智能指针类型 —— `Box<T>` 用于堆分配，`Rc<T>` 用于共享所有权，`Cell<T>`/`RefCell<T>` 用于内部可变性。这些建立在前面章节的所有权和生命周期概念之上。你还将看到 `Weak<T>` 打破引用循环的简要介绍。

**为什么使用 `Box<T>`？** 在 C 中，你使用 `malloc`/`free` 进行堆分配。在 C++ 中，`std::unique_ptr<T>` 包装 `new`/`delete`。Rust 的 `Box<T>` 是等价物 —— 堆分配的、单一所有者的指针，在离开作用域时自动释放。与 `malloc` 不同，没有需要记住的匹配 `free`。与 `unique_ptr` 不同，没有移动后使用 —— 编译器完全防止它。
...
- `Box<T>` 可用于创建指向堆分配类型的指针。无论 `<T>` 的类型如何，指针总是固定大小
```rust
fn main() {
    // 创建指向在堆上创建的整数（值为 42）的指针
    let f = Box::new(42);
    println!("{} {}", *f, f);
    // 克隆盒子创建新的堆分配
    let mut g = f.clone();
...
void c_pointer_problems() {
    ...
    *ptr1 = 42;
    ...
    int* ptr2 = ptr1;  // 都指向相同内存
    int* ptr3 = ptr1;  // 三个指针指向相同内存
...
>     auto ptr2 = std::move(ptr1);  // OK：所有权转移
>     
>     // 但 C++ 仍然允许移动后使用：
>     // std::cout << *ptr1;  // 编译！但是未定义行为！
>     
>     // shared_ptr 别名：
>     auto shared1 = std::make_shared<int>(42);
>     auto shared2 = shared1;  // 都拥有数据
>     // 谁"真正"拥有它？都不是。到处是引用计数开销。
> }
> ```
...
graph TD
    ...
    subgraph "Rust 所有权系统"
        RO1["data: Box<i32>"] --> RM["堆内存<br/>值：42"]
        RO1 -.->|"移动所有权"| RO2["moved_data: Box<i32>"]
        RO2 --> RM
        RO1_X["data: [警告] 已移动<br/>无法访问"]
        RB["&moved_data<br/>不可变借用"] -.->|"安全引用"| RM
        RD["离开作用域时<br/>自动 Drop"] --> RM
    end
...
fn borrowing_rules_example() {
    ...
    ref_mut.push(6);
    // ref1 和 ref2 在 ref_mut 活动时不能使用
    
    // 在 ref_mut 完成后，不可变借用再次有效
    let ref3 = &data;
    println!("{:?}", ref3);
}
```
...
graph TD
    subgraph "Rust 借用规则"
        ...
        subgraph "阶段 2：独占可变借用 [OK]"
            MR["&mut data (ref_mut)"]
            D --> MR
            MR -.->|"独占读/写"| MEM2["内存：[1,2,3,4,5,6]"]
            BLOCK["[错误] 其他借用被阻止"]
        end
        
        subgraph "阶段 3：不可变借用再次 [OK]"
            IR3["&data (ref3)"]
            D --> IR3
    ...
    subgraph "C/C++ 允许的（危险）"
        ...
        CP2 --> CMEM
        CP3 --> CMEM
        RACE["[错误] 可能的数据竞争<br/>[错误] 可能的释放后使用"]
    end
...
回想一下，默认情况下 Rust 中的变量是不可变的。有时希望类型的大部分内容是只读的，同时允许对单个字段进行写访问。

```rust
struct Employee {
    employee_id : u64,   // 这必须是不可变的
    on_vacation: bool,   // 如果我们想允许对这个字段进行写访问，但使 employee_id 不可变呢？
}
```

- 回想一下，Rust 允许对变量有*单个可变*引用和任意数量的*不可变*引用 —— 在*编译时*强制执行
- 如果我们想传递一个*不可变的*员工向量，*但*允许更新 `on_vacation` 字段，同时确保 `employee_id` 不能被可变呢？

### `Cell<T>` —— 用于 Copy 类型的内部可变性

- `Cell<T>` 提供**内部可变性**，即对否则为只读的引用的特定元素的写访问
- 通过复制值进出工作（需要 `T: Copy` 用于 `.get()`）

### `RefCell<T>` —— 带运行时借用检查的内部可变性

- `RefCell<T>` 提供一种适用于引用的变体
    - 在**运行时**而不是编译时强制执行 Rust 借用检查
    - 允许单个*可变*借用，但如果有任何其他引用未解决则**panic**
...
| 失败模式 | 不会失败 —— 无运行时检查 | 如果在活动借用时可变借用则**panic** |
| 开销 | 零 —— 只复制字节 | 小 —— 在运行时跟踪借用状态 |
| 使用时 | 你需要不可变结构体内的可变标志、计数器或小值 | 你需要在不可变结构体内可变 `String`、`Vec` 或复杂类型 |

---

## 共享所有权：`Rc<T>`

`Rc<T>` 允许*不可变*数据的引用计数共享所有权。如果我们想在不复制的情况下将同一个 `Employee` 存储在多个地方呢？
...
> **对于 C++ 开发者：智能指针映射**
>
> | C++ 智能指针 | Rust 等价物 | 主要区别 |
> |---|---|---|
> | `std::unique_ptr<T>` | `Box<T>` | Rust 的版本是默认的 —— 移动是语言级的，不是可选的 |
> | `std::shared_ptr<T>` | `Rc<T>`（单线程）/ `Arc<T>`（多线程） | `Rc` 无原子开销；仅当跨线程共享时使用 `Arc` |
> | `std::weak_ptr<T>` | `Weak<T>`（来自 `Rc::downgrade()` 或 `Arc::downgrade()`） | 相同目的：打破引用循环 |
>
> **关键区别**：在 C++ 中，你*选择*使用智能指针。在 Rust 中，所有值（`T`）和借用（`&T`）覆盖大多数用例 —— 仅当你需要堆分配或共享所有权时才使用 `Box`/`Rc`/`Arc`。

### 使用 `Weak<T>` 打破引用循环

`Rc<T>` 使用引用计数 —— 如果两个 `Rc` 值相互指向，都不会被 drop（循环）。`Weak<T>` 解决了这个问题：
...
struct Node {
    value: i32,
    parent: Option<Weak<Node>>,  // 弱引用 —— 不防止 drop
}
...
> `Weak<T>` 在 [避免过度 clone()](ch17-1-avoiding-excessive-clone.md) 中有更深入的覆盖。现在，关键要点：**在树/图结构中对"后向引用"使用 `Weak` 以避免内存泄漏。**

---

## 组合 `Rc` 与内部可变性

当你组合 `Rc<T>`（共享所有权）与 `Cell<T>` 或 `RefCell<T>`（内部可变性）时，真正的力量出现了。这让多个所有者**读取和修改**共享数据：

| 模式 | 用例 |
|---------|----------|
| `Rc<RefCell<T>>` | 共享、可变数据（单线程） |
| `Arc<Mutex<T>>` | 共享、可变数据（多线程 —— 见 [ch13](ch13-concurrency.md)） |
...
- **第 1 部分（Rc）**：创建一个 `Employee` 结构体，带 `employee_id: u64` 和 `name: String`。将其放入 `Rc<Employee>` 中并克隆到两个独立的 `Vec` 中（`us_employees` 和 `global_employees`）。从两个向量打印以显示它们共享相同的数据。
- **第 2 部分（Cell）**：向 `Employee` 添加 `on_vacation: Cell<bool>` 字段。将不可变 `&Employee` 引用传递给函数并从该函数内部切换 `on_vacation` —— 无需使引用可变。
- **第 3 部分（RefCell）**：用 `name: RefCell<String>` 替换 `name: String`，并编写一个函数，通过 `&Employee`（不可变引用）在员工姓名后追加后缀。
...
fn append_title(emp: &Employee, title: &str) {
    // TODO：通过 RefCell 可变借用 name 并 push_str title
}
