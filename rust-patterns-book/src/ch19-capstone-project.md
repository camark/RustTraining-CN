...
本项目将全书的模式整合到一个生产风格的系统中。你将构建一个**类型安全的并发任务调度器**，使用泛型、traits、类型状态、channel、错误处理和测试。
...
> - 任务生命周期的类型状态模式（第 3 章）
> - PhantomData 用于零成本状态标记（第 4 章）
> - 用于 worker 通信的 channel（第 5 章）
> - 使用作用域线程的并发（第 6 章）
...
2. **Workers** 从 channel 拉取任务，执行它们，并报告结果
...
添加一个 `TaskId` 生成器（简单的原子计数器或 mutex 保护的计数器）。
...
使用 channel（第 5 章）和作用域线程（第 6 章）构建调度器：
...
**你的工作**：实现：
- `Scheduler::new(num_workers: usize) -> Self` —— 创建 channel 并生成 workers
...
1. **快乐路径**：提交 10 个任务，关闭，验证所有 10 个结果都是 `Ok`
2. **错误处理**：提交失败的任务，验证 `TaskResult.outcome` 是 `Err`
3. **空调度器**：创建并立即关闭 —— 无 panic
4. **属性测试**（奖励）：使用 `proptest` 验证对于任意 N 个任务（1..100），调度器总是返回恰好 N 个结果
...
fn main() {
    ...
    for i in 0..20 {
        ...
        // 注意：.unwrap() 用于简洁 —— 生产环境中处理 SendError。
        scheduler.submit(item).unwrap();
    }
...
| 错误处理 | 所有失败捕获在 `TaskResult` 中，无 panic |
| 测试 | 至少 3 个测试；奖励给 proptest |
...
1. **优先级队列**：添加 `Priority` 新类型（1-10）并优先处理更高优先级的任务
2. **重试策略**：失败的任务重试最多 N 次，然后标记为永久失败
3. **取消**：添加 `cancel(TaskId)` 方法，移除待处理任务
4. **异步版本**：移植到 `tokio::spawn` 和 `tokio::sync::mpsc` channel（第 15 章）
