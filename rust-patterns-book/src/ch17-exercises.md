...
使用类型状态模式构建一个交通灯状态机。交通灯必须按 `Red → Green → Yellow → Red` 顺序转换，其他顺序都不应该可能。
...
### 练习 2：使用 PhantomData 的计量单位 ★★（约 30 分钟）
...
使用 channel 构建一个工作池，其中：
- 调度器通过 channel 发送 `Job` 结构体
- N 个工作线程消费作业并发送结果回来
- 使用 `crossbeam-channel`（或 `std::sync::mpsc`，如果 crossbeam 不可用）
...
fn worker_pool(jobs: Vec<Job>, num_workers: usize) -> Vec<JobResult> {
    ...
    drop(result_tx); // 丢弃我们的副本，以便 result channel 在工作线程完成时关闭
    ...
    drop(job_tx); // 关闭 job channel —— 工作线程在清空后退出
...
**进阶**：在阶段之间更改类型的泛型管道将使用不同的设计 —— 每个 `.pipe()` 返回带不同输出类型的 `Pipeline`（这需要更高级的泛型设计）。

</details>

---

### 练习 5：使用 thiserror 的错误层次 ★★（约 30 分钟）
...
设计一个 `Repository<T>` trait，带关联的 `Error` 和 `Id` 类型。为内存存储实现它并演示编译时类型安全。
...
### 练习 7：Unsafe 的安全包装器（第 11 章）★★★（约 45 分钟）
...
- 所有公共方法必须是安全的；所有 unsafe 必须被封装，带 `SAFETY:` 注释
- `Drop` 必须清理已初始化的元素
...
- 支持尾随逗号
- 支持空调用 `map!{}`
- 适用于任何实现 `Into<K>` 和 `Into<V>` 的类型以获得最大的灵活性
...
设计一个 `Duration` 包装器，使用自定义 serde 反序列化器从人类可读的字符串如 `"30s"`、`"5m"`、`"2h"` 反序列化。该结构体也应该序列化为相同的格式。
...
编写一个异步函数 `fetch_all`，生成三个 `tokio::spawn` 任务，每个任务用 `tokio::time::sleep` 模拟网络调用。用 `tokio::try_join!` 连接所有三个，并用 `tokio::time::timeout(Duration::from_secs(5), ...)` 包装。返回 `Result<Vec<String>, ...>` 或任何任务失败或到期时的错误。
...
将整个 `try_join!` 包装在 `timeout()` 中 —— `Elapsed` 错误意味着到达截止时间。
...
### 练习 11 —— 异步 Channel 管道 ★★★（约 40 分钟）

使用 `tokio::sync::mpsc` 构建一个 producer → transformer → consumer 管道：

1. **Producer**：发送整数 1..=20 到 channel A（容量 4）。
2. **Transformer**：从 channel A 读取，平方每个值，发送到 channel B。
3. **Consumer**：从 channel B 读取，收集到 `Vec<u64>`，返回它。

所有三个阶段作为并发 `tokio::spawn` 任务运行。使用有界 channel 演示背压。断言最终的 vec 等于 `[1, 4, 9, ..., 400]`。

**学习目标**：`mpsc::channel`、有界背压、带 move 闭包的 `tokio::spawn`、通过 channel 关闭的优雅关闭。
