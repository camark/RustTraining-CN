...
| 工作 | 示例 | 它做什么 |
|------|------|---------|
| **生命周期标记** | `PhantomData<&'a T>` | 结构体被视为借用 `'a` |
| **所有权模拟** | `PhantomData<T>` | Drop 检查假设结构体拥有一个 `T` |
| **方差控制** | `PhantomData<fn(T)>` | 使结构体在 `T` 上是逆变的 |
...
fn main() {
    ...
    // 不能将 handle1 与不同的 arena 一起使用 —— 生命周期不匹配
...
fn main() {
    ...
    // let nonsense = dist + time; // ❌ 编译错误：不能添加米 + 秒
}
```

> **这是纯类型系统魔法** —— `PhantomData<Meters>` 是零大小的，
> 所以 `Quantity<Meters>` 与 `f64` 布局相同。运行时无包装开销，
> 但编译时有完整单位安全。

### PhantomData 和 Drop Check

当编译器检查结构体的析构函数是否可能访问过期数据时，它使用 `PhantomData` 决定：
...
**实践规则**：当包装原始指针时，仔细选择 PhantomData：
- 编写拥有其数据的容器？ → `PhantomData<T>`
- 编写视图/引用类型？ → `PhantomData<&'a T>` 或 `PhantomData<*const T>`

### 方差 —— 为什么 PhantomData 的类型参数重要

**方差**决定泛型类型是否可以用子类型或超类型替换（在 Rust 中，"子类型"意味着"有更长的生命周期"）。弄错方差会导致拒绝好代码或接受不健全代码。
...
| 方差 | 含义 | "我可以替换吗…" | Rust 示例 |
|------|------|----------------|----------|
| **协变** | 子类型流经 | `'long` 在期望 `'short` 处 ✅ | `&'a T`、`Vec<T>`、`Box<T>` |
| **逆变** | 子类型*反向*流动 | `'short` 在期望 `'long` 处 ✅ | `fn(T)`（在参数位置） |
| **不变** | 不允许替换 | 两个方向都不 ✅ | `&mut T`、`Cell<T>`、`UnsafeCell<T>` |
...
fn main() {
    ...
    // owned 存活整个函数（'long）
    // print_str 期望 &'_ str（'short —— 仅调用）
    print_str(&owned); // ✅ 协变：'long → 'short 是安全的
    // 更长生命周期的引用总是可以在需要较短引用时使用。
}
```
...
fn evil(s: &mut &'static str) {
    // 我们可以将较短生命周期的 &str 写入 &'static str 槽！
    let local = String::from("temporary");
    // *s = &local; // ← 会创建悬空 &'static str
}
...
| `PhantomData<&'a T>` | 协变 | 协变 | 你用生命周期 `'a` 借用一个 `T` |
...
fn main() {
    ...
    use_token(&token); // ✅ 工作因为 SessionToken 在 'a 上是协变的
}
```

> **决策规则**：从 `PhantomData<&'a T>`（协变）开始。仅当你的抽象分发可变访问到 `T` 时才切换到 `PhantomData<&'a mut T>`（不变）。几乎从不使用 `PhantomData<fn(T)>`（逆变） —— 它仅对回调存储场景正确。

> **关键要点 —— PhantomData**
> - `PhantomData<T>` 携带类型/生命周期信息，无运行时成本
> - 用它于生命周期标记、方差控制和单位测量模式
> - Drop 检查：`PhantomData<T>` 告诉编译器你的类型在逻辑上拥有一个 `T`

> **另见：**[第 3 章 — Newtype 和类型状态](ch03-the-newtype-and-type-state-patterns.md) 了解使用 PhantomData 的类型状态模式。[第 12 章 — Unsafe Rust](ch12-unsafe-rust-controlled-danger.md) 了解 PhantomData 如何与原始指针交互。

---

### 练习：带 PhantomData 的单位测量 ★★（约 30 分钟）
...
fn main() {
    ...
    // let bad = width + time; // ❌ 编译错误：不能添加米 + 秒
}
```
