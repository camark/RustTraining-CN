...
struct ServerConfig {
    name: String,
    port: u16,
    #[serde(default)]                    // 如果缺失使用 Default::default()
...
> **关键洞察**：你的结构体 derive `Serialize` 和 `Deserialize` 一次。
> 然后它适用于*每个* serde 兼容的格式 —— JSON、TOML、YAML、
> bincode、MessagePack、CBOR、postcard 和数十种更多。
...
| 属性 | 级别 | 效果 |
|-----------|-------|--------|
| `rename_all = "camelCase"` | 容器 | 重命名所有字段为 camelCase/snake_case/SCREAMING_SNAKE_CASE |
| `deny_unknown_fields` | 容器 | 对意外键报错（严格模式） |
...
> **选择哪种表示**：对大多数 JSON API 使用内部标记（`tag = "type"`） —— 它最易读且符合 Go、Python 和 TypeScript 中的约定。仅对"联合"类型使用无标记，其中形状本身消除歧义。

### 零拷贝反序列化

serde 可以不经分配新字符串反序列化 —— 直接从输入缓冲区借用。这是高性能解析的关键：

```rust,ignore
use serde::Deserialize;

// --- 所有（分配） ---
// 每个 String 字段将字节从输入复制到新堆分配。
#[derive(Deserialize)]
struct OwnedRecord {
    name: String,           // 分配新 String
...
> **实用提示**：`Cow<'a, str>` 给你两全其美 —— 可能时借用，必要时分配（例如，当 JSON 转义序列需要解转义时）。serde 原生支持 Cow。
...
| TOML | `toml` | ✅ | 中等 | 好 | 配置文件（Cargo.toml 风格） |
| YAML | `serde_yaml` | ✅ | 中等 | 好 | 配置文件（复杂嵌套） |
...
// bincode: 紧凑二进制 —— ~40 字节，无字段名
let bin = bincode::serialize(&config).unwrap();            // 更小

// postcard: 甚至更小，varint 编码 —— 适合嵌入式
...
对于硬件诊断，解析二进制协议数据很常见。Rust 提供工具用于安全、零拷贝的二进制数据处理：

```rust
// --- #[repr(C)]：可预测的内存布局 ---
// 确保字段按声明顺序布局，带 C 填充规则。
// 对匹配硬件寄存器布局和协议头至关重要。
...
struct PcieCapabilityHeader {
    ...
    next_cap: u8,      // 指向下一个 capability
    cap_reg: u16,      // 特定于 capability 的寄存器
}
// ⚠️  packed 结构体：获取 &field 创建未对齐引用 —— UB。
// 总是复制字段出来：let id = header.cap_id;  // OK (Copy)
// 绝不这样做：let r = &header.cap_reg;               // UB 如果未对齐
```

### zerocopy 和 bytemuck —— 安全转换

不使用 `unsafe` transmute，使用在编译时验证布局安全的 crates：
...
| `unsafe { transmute() }` | ❌ 不安全 | 零拷贝 | 最后手段 —— 避免在应用代码中使用 |

### bytes::Bytes —— 引用计数缓冲区

`bytes` crate（被 tokio、hyper、tonic 使用）提供带引用计数的零拷贝字节缓冲区 —— `Bytes` 对 `Vec<u8>` 就像 `Arc<[u8]>` 对所有切片：
...
| 子切片 | 带生命周期借用 | 所有的、引用计数追踪的 |
| 线程安全 | 非 `Sync`（需要 `Arc`） | 内置 `Send + Sync` |
| 可变性 | 直接 `&mut` | 先拆分为 `BytesMut` |
| 生态系统 | 标准库 | tokio、hyper、tonic、axum |

> **何时使用 bytes**：网络协议、包解析、任何你需要接收缓冲区并将其拆分为由不同组件或线程处理的部分的场景。零拷贝拆分是杀手级特性。

> **关键要点 —— 序列化和二进制数据**
> - serde 的 derive 宏处理 90% 的情况；对其余使用属性（`rename`、`skip`、`default`）
> - 零拷贝反序列化（结构体中的 `&'a str`）避免读密集型工作负载的分配
> - `repr(C)` + `zerocopy`/`bytemuck` 用于硬件寄存器布局；`bytes::Bytes` 用于引用计数缓冲区

> **另见：**[第 10 章 —— 错误处理](ch10-error-handling-patterns.md) 了解将 serde 错误与 `thiserror` 结合。[第 12 章 —— Unsafe](ch12-unsafe-rust-controlled-danger.md) 了解 `repr(C)` 和 FFI 数据布局。
