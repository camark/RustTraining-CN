...
> **你将学到什么：**
> - 五种 unsafe 超能力以及何时需要每种
> - 编写可靠的抽象：安全的 API，不安全的内部实现
> - 从 Rust 调用 C（以及返回）的 FFI 模式
> - 常见 UB 陷阱和 arena/slab 分配器模式

## 五种 Unsafe 超能力

`unsafe` 解锁编译器无法验证的五种操作：
...
`unsafe` 的目的是在unsafe操作周围构建**安全的抽象**：
...
impl<T, const N: usize> StackBuf<T, N> {
    pub fn new() -> Self {
        StackBuf {
            // 每个元素单独 MaybeUninit —— 无需 unsafe。
            // `const { ... }` 块（Rust 1.79+）让我们重复非 Copy 的
            // const 表达式 N 次。
    ...
    pub fn push(&mut self, value: T) -> Result<(), T> {
        if self.len >= N {
            return Err(value); // 缓冲区满 —— 返回值给调用者
        }
        // 安全：len < N，所以 data[len] 在边界内。
        // 我们将有效的 T 写入 MaybeUninit 槽。
    ...
    pub fn get(&self, index: usize) -> Option<&T> {
        if index < self.len {
            // 安全：index < len，且 data[0..len] 都已初始化。
...
**可靠 unsafe 代码的三条规则**：
1. **文档化不变量** —— 每个 `// SAFETY:` 注释解释为什么操作有效
2. **封装** —— unsafe 在安全 API 内部；用户不能触发 UB
...
fn safe_strlen(s: &str) -> usize {
    ...
    // 安全：c_string 是有效的空终止字符串，在调用期间存活。
...
| `std::ffi::CString` | `char*`（所有） | 空终止，所有 |
...
| 陷阱 | 示例 | 为什么是 UB |
|---------|---------|------------|
| 空解引用 | `*std::ptr::null::<i32>()` | 解引用空总是 UB |
| 悬空指针 | `drop()` 后解引用 | 内存可能被重用 |
| 数据竞争 | 两个线程写入 `static mut` | 无同步的并发写入 |
| 错误的 `assume_init` | `MaybeUninit::<String>::uninit().assume_init()` | 读取未初始化内存。**注意**：`[const { MaybeUninit::uninit() }; N]`（Rust 1.79+）是创建 `MaybeUninit` 数组的安全方式 —— 无需 `unsafe` 或 `assume_init`（见上面的 `StackBuf::new()`）。 |
| 别名违规 | 创建两个 `&mut` 指向相同数据 | 违反 Rust 的别名模型 |
...
在 C 中，你会为特定分配模式编写自定义 `malloc()` 替代品 —— 一次性释放所有内容的 arena 分配器、用于固定大小对象的 slab 分配器、或用于高吞吐量系统的池分配器。Rust 通过 `GlobalAlloc` trait 和分配器 crate 提供相同的能力，并带有生命周期作用域 arena 的额外好处，**在编译时防止释放后使用**。

#### Arena 分配器 —— 批量分配，批量释放

Arena 通过向前推进指针来分配。单个项不能被释放 —— 整个 arena 一次性释放。这非常适合请求作用域或帧作用域的分配：
...
| 释放速度 | 每个对象析构函数 | O(1) 批量释放 |
| 碎片 | 是（长生命周期进程） | arena 内无 |
| 生命周期安全 | 堆 —— `Drop` 时释放 | arena 引用 —— 编译时作用域 |
| 使用场景 | 通用 | 请求/帧/批处理 |

#### `typed-arena` —— 类型安全的 Arena

当所有 arena 对象是相同类型时，`typed-arena` 提供更简单的 API，返回带 arena 生命周期的引用：
...
slab 分配器预先分配固定大小槽的池。对象单独分配和返回，但所有槽大小相同 —— 消除碎片并实现 O(1) 分配/释放：
...
fn connection_pool_example() {
    ...
    // remove 返回值 —— O(1)，槽用于下一次 insert 重用
    let removed = connections.remove(key2);
    assert_eq!(removed.id, 1002);

    // 下一次 insert 重用释放的槽 —— 无碎片
    let key3 = connections.insert(Connection {
        id: 1003,
        buffer: [0; 1024],
        active: true,
    });
    assert_eq!(key3, key2); // 重用相同槽！
}
```

#### 实现最小 Arena（用于 `no_std`）

对于不能引入 `bumpalo` 的裸机环境，这里有一个基于 `unsafe` 构建的最小 arena：
...
/// 非线程安全 —— 在多线程上下文中每核心使用或带锁使用。
///
/// **重要**：像 `bumpalo` 一样，这个 arena 在 arena 被 drop 时
/// 不调用分配项的析构函数。有 `Drop` impl 的类型将
/// 泄漏它们的资源（文件句柄、套接字等）。只分配没有
/// 有意义 `Drop` impl 的类型，或在 arena 之前手动 drop 它们。
pub struct FixedArena<const N: usize> {
    // UnsafeCell 在这里是必需的：我们通过 `&self` 可变 `buf`。
...
impl<const N: usize> FixedArena<N> {
    ...
    pub fn alloc<T>(&self, value: T) -> Option<&mut T> {
        ...
        // 安全：
        // - `aligned` 在 `buf` 边界内（上面检查）
        // - 对齐正确（对齐到 T 的要求）
        // - 无别名：每个分配返回唯一的、不重叠的区域
        // - UnsafeCell 授予通过 &self 可变的权限
        // - arena 比返回的引用活得长（调用者必须确保）
        let ptr = unsafe {
...
> **交叉引用**：对于裸机分配器设置（带 `embedded-alloc` 的 `#[global_allocator]`），见*Rust Training for C Programmers*第 15.1 章"Global Allocator Setup"，其中涵盖嵌入式特定的引导。

> **关键要点 —— Unsafe Rust**
> - 文档化不变量（`SAFETY:` 注释），封装在安全 API 后，最小化 unsafe 范围
> - `[const { MaybeUninit::uninit() }; N]`（Rust 1.79+）替换旧的 `assume_init` 反模式
> - FFI 需要 `extern "C"`、`#[repr(C)]` 和仔细的 null/生命周期处理
> - Arena 和 slab 分配器用通用灵活性换取分配速度

> **另见：**[第 4 章 —— PhantomData](ch04-phantomdata-types-that-carry-no-data.md) 了解 variance 和 drop-check 与 unsafe 代码的交互。[第 9 章 —— 智能指针](ch09-smart-pointers-and-interior-mutability.md) 了解 Pin 和自引用类型。

---

### 练习：围绕 Unsafe 的安全包装器 ★★★（约 45 分钟）

编写一个 `FixedVec<T, const N: usize>` —— 固定容量、栈分配的 vector。
要求：
...
impl<T, const N: usize> FixedVec<T, N> {
    ...
    pub fn pop(&mut self) -> Option<T> {
        ...
        self.len -= 1;
        // 安全：data[len] 已初始化（len 在递减前 > 0）。
        Some(unsafe { self.data[self.len].assume_init_read() })
    }

    pub fn as_slice(&self) -> &[T] {
        // 安全：data[0..len] 都已初始化，且 MaybeUninit<T>
        // 与 T 布局相同。
