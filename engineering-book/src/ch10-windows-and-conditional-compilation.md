...
>
> **交叉引用：** [`no_std` & 功能](ch09-no-std-and-feature-verification.md) —— `cargo-hack` 和功能验证 · [跨平台编译](ch02-cross-compilation-one-source-many-target.md) —— 通用跨构建设置 · [构建脚本](ch01-build-scripts-buildrs-in-depth.md) —— `build.rs` 发出的 `cfg` 标志

### Windows 支持 —— 平台抽象

Rust 的 `#[cfg()]` 属性和 Cargo 功能允许单个代码库干净地针对 Linux 和 Windows。项目已经在 `platform::run_command` 中演示了此模式：

```rust
// 来自项目的真实模式 —— 平台特定 shell 调用
...
# 固定到单个次要版本 —— 每个发布版本可能移除或重命名 API 绑定。
# 在开始新项目之前，检查 https://github.com/microsoft/windows-rs 获取最新版本。

# 或使用 windows crate 获取安全包装器（更重，更 ergonomic）
...
| 二进制大小 | 最小（仅外部声明） | 更大（包装器代码） |
| 编译时间 | 快 | 更慢 |
| 人体工程学 | C 风格，手动安全 | Rust 习惯用法 |
| 错误处理 | 原始 `BOOL` / `HRESULT` | `Result<T, windows::core::Error>` |
| 使用时 | 性能关键，薄包装器 | 应用程序代码，易用性 |
...
# 选项 2：通过 xwin 的 MSVC ABI（用于完整 MSVC 兼容性）
...
```rust
// src/platform/mod.rs —— 每个 OS 编译不同的模块
#[cfg(target_os = "linux")]
mod linux;
#[cfg(target_os = "linux")]
pub use linux::*;
...
```rust
// 如果有人试图在没有功能的情况下为 Windows 构建则编译错误：
#[cfg(all(target_os = "windows", not(feature = "windows")))]
compile_error!("为 Windows 构建请启用 'windows' 功能");
```
...
项目已经部分为 Windows 做好准备。使用 [`cargo-hack`](ch09-no-std-and-feature-verification.md) 验证所有功能组合，并 [跨平台编译](ch02-cross-compilation-one-source-many-target.md) 以在 Linux 上测试 Windows：

**已经完成：**
- `platform::run_command` 使用 `#[cfg(windows)]` 进行 shell 选择
- 测试对平台适当的测试命令使用 `#[cfg(windows)]` / `#[cfg(not(windows))]`
...
阶段 1：提取平台抽象 trait（当前 → 2 周）
  ...
  ├─ 在当前 Linux 代码后包装 LinuxHardware impl
  └─ 所有诊断模块依赖于 trait，而非 Linux 特定
...
阶段 3：Windows 实现（持续）
  ├─ 通过 ipmiutil.exe 或 OpenIPMI Windows 驱动的 IPMI
...
这确保代码库在 Windows 完全实现完成之前就能在 Windows 上编译 —— 及早捕获 `cfg` 错误。

> **关键洞察**：抽象不需要在第一天就完美。从叶函数中的 `#[cfg]` 块开始（像 `exec_cmd` 已经做的那样），然后当你有两个或更多平台实现时重构为 trait。过早的抽象比 `#[cfg]` 块更糟糕。

### 条件编译决策树

```mermaid
flowchart TD
    START["平台特定代码？"] --> HOW_MANY{"多少平台？"}
    
    HOW_MANY -->|"2（Linux + Windows）"| CFG_BLOCKS["#[cfg] 块\n在叶函数中"]
    HOW_MANY -->|"3+"| TRAIT_APPROACH["Platform trait\n+ 每平台实现"]
    
    CFG_BLOCKS --> WINAPI{"需要 Windows API？"}
    WINAPI -->|"最小"| WIN_SYS["windows-sys\n原始 FFI 绑定"]
    WINAPI -->|"丰富（COM 等）"| WIN_RS["windows crate\n安全习惯包装器"]
    WINAPI -->|"无\n(仅 #[cfg])"| NATIVE["cfg(windows)\ncfg(unix)"]
    
    TRAIT_APPROACH --> CI_CHECK["cargo-hack\n--each-feature"]
    CFG_BLOCKS --> CI_CHECK
    CI_CHECK --> XCOMPILE["CI 中跨平台编译\ncargo-xwin 或\n原生运行器"]
...
创建一个带 `#[cfg(unix)]` 和 `#[cfg(windows)]` 实现的 `get_hostname()` 函数模块。用 `cargo check` 和 `cargo check --target x86_64-pc-windows-msvc` 验证两者编译。
...
安装 `cargo-xwin` 并从 Linux 为 `x86_64-pc-windows-msvc` 构建简单的二进制文件。验证输出是 `.exe`。
...
- 从叶函数中的 `#[cfg]` 块开始；仅当三个或更多平台分歧时才重构为 trait
- `windows-sys` 用于原始 FFI；`windows` crate 提供安全、习惯的包装器
- `cargo-xwin` 从 Linux 跨平台编译到 Windows MSVC ABI —— 无需 Windows 机器
- 即使在 CI 中只发布 Linux，也要始终检查 `--target x86_64-pc-windows-msvc`
- 将 `#[cfg]` 与 Cargo 功能结合用于可选平台支持（例如 `feature = "windows"`)
