...
```bash
# ─── 构建脚本 ───
cargo build                          # 先编译 build.rs，然后 crate
...
# ─── 审计和供应链 ───
cargo audit                          # 已知漏洞扫描
cargo audit --deny warnings          # 在任何咨询时失败 CI
cargo deny check                     # 许可证 + 咨询 + 禁止 + 源检查
...
# ─── 二进制优化 ───
cargo bloat --release --crates       # 每 crate 的大小贡献
...
cargo nextest run --retries 2        # 重试不稳定测试
...
cfg!(target_os = "linux")                    # 编译时 cfg（评估为 bool）

# ─── 发布 ───
cargo release patch --dry-run        # 预览发布
cargo release patch --execute        # 提升、提交、打标签、发布
cargo dist plan                      # 预览分发生成物
```
...
| 目标 | 工具 | 何时使用 |
|------|------|-------------|
| 嵌入 git hash / 构建信息 | `build.rs` | 二进制需要可追溯性 |
| 与 Rust 一起编译 C 代码 | `build.rs` 中的 `cc` crate | 小型 C 库的 FFI |
...
| 定位旧 glibc | `cargo-zigbuild` | RHEL 7、CentOS 7 兼容性 |
| ARM 服务器二进制文件 | `cross` 或 `cargo-zigbuild` | Graviton/Ampere 部署 |
| 统计基准测试 | Criterion.rs | 性能回归检测 |
| 快速性能检查 | Divan | 开发时性能分析 |
| 发现热点 | `cargo flamegraph` / `perf` | 基准测试识别慢代码后 |
| 行/分支覆盖率 | `cargo-llvm-cov` | CI 覆盖率门禁、差距分析 |
...
| 泄漏检测 | Valgrind 或 LSan | 长期运行的服务 |
| 本地 CI 等价物 | `cargo-make` | 开发者工作流自动化 |
| Pre-commit 检查 | `cargo-husky` 或 git 钩子 | 在推送前捕获问题 |
| 自动发布 | `cargo-release` + `cargo-dist` | 版本管理 + 分发 |
| 依赖审计 | `cargo-audit` / `cargo-deny` | 供应链安全 |
| 许可证合规 | `cargo-deny`（许可证） | 商业/企业项目 |
| 供应链信任 | `cargo-vet` | 高安全环境 |
| 查找过时依赖 | `cargo-outdated` | 计划维护 |
| 检测破坏性变更 | `cargo-semver-checks` | 库 crate 发布 |
| 依赖树分析 | `cargo tree --duplicates` | 去重和修剪依赖图 |
| 二进制大小分析 | `cargo-bloat` | 大小受限的部署 |
| 查找未使用依赖 | `cargo-udeps` / `cargo-machete` | 修剪编译时间和大小 |
...
| 并发模型检查 | `loom` | 无锁数据结构、原子排序 |
| 功能组合测试 | `cargo-hack` | 带多个 `#[cfg]` 功能的 crate |
| 快速 UB 检查（接近原生） | `cargo-careful` | CI 安全门，比 Miri 更轻量 |
| 保存时自动重新构建 | `cargo-watch` | 开发者内部循环、紧密反馈 |
| 工作空间文档 | `cargo doc` + rustdoc | API 发现、入职、文档链接 CI |
| 可重现构建 | `--locked` + `SOURCE_DATE_EPOCH` | 发布完整性验证 |
| CI 缓存调优 | `Swatinem/rust-cache@v2` | 构建时间减少（冷 → 缓存） |
| 工作空间 lint 策略 | Cargo.toml 中的 `[workspace.lints]` | 所有 crate 一致的 Clippy/编译器 lint |
| 自动修复 lint 警告 | `cargo clippy --fix` | 琐碎问题的自动清理 |
...
| Rust 中的消毒器 | [rustc Sanitizer docs](https://doc.rust-lang.org/nightly/unstable-book/compiler-flags/sanitizer.html) |
...
| 性能分析引导优化 | [Rust PGO 指南](https://doc.rust-lang.org/rustc/profile-guided-optimization.html) |
...
| Cargo 配置文件 | [Cargo Book — Profiles](https://doc.rust-lang.org/cargo/reference/profiles.html) |
...
| Rustdoc 书 | [Rustdoc Book](https://doc.rust-lang.org/rustdoc/) |
...
*生成作为伴侣参考 —— Rust Patterns 和 Type-Driven Correctness 的伴侣。*

*版本 1.3 —— 添加 cargo-hack、cargo-careful、cargo-watch、cargo doc、可重现构建、CI 缓存策略、顶石练习和章节依赖图以完整。*
